...
from tests.e2e.lib.manifests import iter_manifests, load_manifest, load_manifest_ids, manifest_availability
from tests.e2e.lib.paths import WorkspacePaths


def test_iter_manifests_returns_sorted_inventory(workspace_paths: WorkspacePaths) -> None:
    manifests = iter_manifests(workspace_paths)
...
def test_manifest_availability_reads_campaign_and_cell_shapes(workspace_paths: WorkspacePaths) -> None:
    campaign = load_manifest(workspace_paths, "kad2.campaign.startup-and-bootstrap.v1")
    planned_cell = load_manifest(workspace_paths, "ed2k.cell.listener.plaintext.inbound.queue-only.fresh.private.v1")
